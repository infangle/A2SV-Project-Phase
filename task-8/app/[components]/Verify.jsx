...
const Verify = () => {
  ...
  const email = "user@example.com"; // Replace this with the actual email
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Handle successful verification (e.g., redirect to login or home page)
        router.push("/login");
      }
    } catch (error) {
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await fetch("https://akil-backend.onrender.com/resend-code", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!res.ok) {
        ...
        // Optionally handle successful resend
      }
    } catch (error) {
      setErrorMessage("An error occurred. Please try again.");
    }
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className="flex flex-col items-center justify-center p-8 rounded-lg shadow-lg w-full max-w-md">
